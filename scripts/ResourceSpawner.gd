...
func remove_resource_at(cell: Vector2i) -> bool:
	for resource_id in _resolved_order():
		var map: Dictionary = _world.resources.by_cell(resource_id)
		if not map.has(cell):
			continue
...
func _resolved_order() -> Array[String]:
	if not _world.resources.order.is_empty():
		return _world.resources.order
...
func _spawn_resource(resource_id: String) -> void:
	...
	if grid.grid_width < size or grid.grid_height < size:
		return
...
func _pick_cell(def: Dictionary, require_zone: bool) -> Vector2i:
	...
	var grid := _world.grid
	var min_x: int = 0
	var max_x: int = grid.grid_width - size
	var min_y: int = 0
	var max_y: int = grid.grid_height - size
	if require_zone:
		var zone: String = str(def.get("ensure_zone", ""))
		if zone == "player_zone":
			min_x = max(grid.grid_width - grid.player_zone_width, 0)
			max_x = max(grid.grid_width - size, 0)
		elif zone == "stone_band":
			var band: Vector2i = grid.stone_band_bounds()
			min_x = band.x
			max_x = band.y
...
func _place_resource(resource_id: String, top_left: Vector2i) -> bool:
	...
	node.set("cell_size", grid.cell_size)
	...
	for x in range(size):
		for y in range(size):
			...
			if path.is_path_cell(cell):
				return false
			if occupancy.is_enemy_tower_cell(cell):
				return false
...
func _cell_in_zone(cell: Vector2i, zone: String) -> bool:
	var grid := _world.grid
	if zone == "player_zone":
		return grid.is_in_player_zone(cell)
	if zone == "stone_band":
		return grid.is_in_stone_band(cell)
	return true

func _seed_rng(resource_id: String) -> void:
	if _world.config.random_seed == 0:
		rng.randomize()
	else:
		var offset: int = abs(resource_id.hash()) % 997
		rng.seed = _world.config.random_seed + offset
